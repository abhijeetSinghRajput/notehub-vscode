...
export function showNotePanel(
  ...
  const panel = vscode.window.createWebviewPanel(
    ...
    {
      enableScripts: true,
      // Fix: this was [] before, which told the webview it had NO folder
      // it's allowed to load local resources from — so note-view.css was
      // silently blocked even though the <link> tag pointed at a valid URI.
  ...
  panel.webview.html = renderNoteHtml(panel.webview, note, author, cssUri);
...
function renderNoteHtml(
  ...
  const csp = [
    `default-src 'none'`,
    `img-src https: data:`,
    // Fix: added webview.cspSource so styles served from vscode-webview://
    // (our cssUri) are actually allowed to load, not just https: URLs.
